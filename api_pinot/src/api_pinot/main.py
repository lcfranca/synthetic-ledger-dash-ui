...
async def dashboard_filter_search(
    ...
    return {"field": field, "query": query, "matches": matches, "count": len(matches), "backend": "pinot"}
...
async def ws_dashboard(ws: WebSocket):
    ...
    try:
        while True:
            ...
            await ws.send_json(
                {
                    "event_id": f"dashboard-pinot-{int(datetime.now(timezone.utc).timestamp() * 1000)}",
