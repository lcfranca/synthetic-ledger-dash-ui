...
async def ws_dashboard(ws: WebSocket):
    ...
    try:
        while True:
            ...
            await ws.send_json(
                {
                    "event_id": f"dashboard-druid-{int(datetime.now(timezone.utc).timestamp() * 1000)}",
